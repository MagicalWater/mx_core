...
//    print('是否需要頂部高度: $needHeightSpace, 頂部起始: $top');
  ...
  List<_VerticalSpace> cut(_VerticalSpace other) {
    ...
    var topHeight = intersectSpace.top - top;
    var rightSpan = (x + span) - (intersectSpace.x + intersectSpace.span);
    var bottomHeight =
        (realTop + height) - (intersectSpace.realTop + intersectSpace.height);
...
//    print("結果是: $remainings");
  ...
  _VerticalSpace intersect(_VerticalSpace other) {
    var rect = toRect().intersect(other.toRect());
    return _VerticalSpace(
      x: rect.left.toInt(),
  ...
  bool contains(_VerticalSpace other) {
    var isXContain = (x <= other.x) && (x + span >= other.x + other.span);
    var isYContain = (top <= other.top) &&
        (realTop + height >= other.realTop + other.height);
